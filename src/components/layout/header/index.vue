  ...
  <aside class="sidebar" :class="{ 'sidebar-open': isMobileOpen }">
    ...
    <div class="sidebar-menu-area">
      <ul class="sidebar-menu">
        ...
        <!-- TODO: Section for Courses and Can be Dropdown -->
         <!-- Courses Section -->
          <li :class="{ dropdown: true, open: activeDropdown === 'courses' }">
          <a href="javascript:void(0)" @click="toggleDropdown('courses')" :class="{ active: isDashboardActive }">
            <iconify-icon icon="heroicons:document" class="menu-icon"></iconify-icon>
            <span class="dropdown-arrow" :class="{ rotated: activeDropdown === 'courses' }">Courses</span>
          </a>
          <transition @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter" @before-leave="beforeLeave"
            @leave="leave" @after-leave="afterLeave">
            <ul v-show="activeDropdown === 'courses'" ref="coursesMenu" class="sidebar-submenu">
              <li v-for="item in coursesItems" :key="item.path" :class="['nav-link', { 'active-page': isActive(item.path) }]">
                <router-link :to="item.path">
...
const dashboardItems = [
  ...
  { path: '/', label: 'LMS', colorClass: 'text-purple w-auto' },
...
const coursesItems = [
  { path: '/student-courses', label: 'Student Courses', colorClass: 'text-primary-600 w-auto' },
  { path: '/mentor', label: 'Mentor Courses', colorClass: 'text-warning-main w-auto' },
  { path: '/create', label: 'Create Course', colorClass: 'text-info-main w-auto' },
]
// const invoiceItems = [
...
const settingItems = [
  { path: '/sign-in', label: 'Logout', colorClass: 'text-red w-auto' },
  { path: '/forgot-password', label: 'Forgot Password', colorClass: 'text-yellow w-auto' },
